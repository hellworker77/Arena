...
import type {AppRoute, AppRouteContainer} from "../routing/appRoute.ts";
...
const findRouteBySegments = (routes: AppRoute[], segments: string[]): AppRoute | null => {
    ...
    const route = routes
        .find(r => r.path.replace(/^\/+/, "") === current) ?? null;
...
const getRedirectPath = (route: AppRoute, parentPath = ""): string | null => {
    ...
    if (route.subroutes?.length) return `${pathPrefix}/${route.path}/${route.subroutes[0].path}`;
    return `${pathPrefix}${route.path}`;
};

export const useRedirectToFirstChild = (isAuthorized: boolean, container: AppRouteContainer = "side") => {
    ...
    const { routeToRedirect } = useMemo(() => {
        ...
        const filteredRoutes = app_routes
            .filter(route => route.container === container || route.container === "both")
            .filter(route => !route.authorizationRequired || isAuthorized);

        const currentRoute = findRouteBySegments(filteredRoutes, segments);
